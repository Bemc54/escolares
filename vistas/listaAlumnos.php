    ...
    if (!isset($_SESSION['rol'])){
        ...
        if (isset($_SESSION['rol']) && $_SESSION['rol'] == 'sx') {
            if ($_SESSION['rol'] == 'sx') {
                ...
                $btnAgregar = '
                    <button id="add" style="bottom: 3rem; right: 3rem;" class="btn btn-outline-light border border-light" onclick="registrarAlumno()"><span class="fa fa-user-plus"></span></button>
                ';
            }
            echo '
        ...
        foreach ($lista as $row => $item) {
            $acciones = '
                <td style="justify-content:center; display: flex;">
                    ...
                    <a class="btn btn-icon btn-danger" href="index.php?seccion=listaAlumnos&accion=eliminar&id=' . $item[0] . '"><i class="fa fa-trash"></i></a>
                </td>';
            $contenido = '
